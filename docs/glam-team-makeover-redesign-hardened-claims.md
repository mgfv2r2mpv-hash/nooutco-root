...
- **D-A — Independence by wait-window.** A pass scores `independent` only if the learner activates
  ...
  **or (ii) any prompt delivered at/after budget-exhaustion** (the learner is at the wall and cannot
  ...
  needed a **BT** prompt (or any prompt at exhaustion) this turn is scored non-independent **durably**
  — one more token action does **not** launder it back to `independent` (**AC-23**). "prompt
...
- **D-B — Over-cap refuse+flag+prompt.** An over-cap action on the learner's own turn is
  ...
  index.html:481/:522, where `handoff` climbs `bonus` +1 per turn to a cap of 3, index.html:578 —
  eval F-14/F-36). The extra-time "reinforcer" (§3.8), if kept, is a **non-cap** reward
...
scores `independent=!cueVisible` off a button press with *nothing binding the press to the learner* —
the eval names this exactly (§7.2b: "`independent` here means the BT tapped the button within 3.5 s").
...
the session cannot leave, while D-D's "auto-scale actions to finish the look" rests on actions == progress
— which AC-6 explicitly severs. What happens when a required step is available but the learner will not do
...
suggested "incomplete caps the tier" (that would contradict the lock). Instead I split the outro —
the **tier number** stays turn-taking-only (AC-11), and **completion gates a separate completion
...
(teamwork only, no event-success) + a completion-gated **completion beat** (the event-success flavor)
— **keeping the six approved events unchanged**. This is a correction toward §3.7.1's own
...
a stalling learner is scored `prompted`/`staff-prompted` via the BT path **regardless**" (prompt durable)
— both cannot hold. The design must decide whether a delivered prompt is opportunity-scoped or
...
- (ii) **Scoring inversion via AC-25.** If the learner correctly notices the forget and asks anyway, AC-25
  ...
  clinically *correct* mand under the contrived MO — the entire target of the interrupted-chain program —
  is recorded as a premature interruption, **never `independent`**. The right behavior is filed as the
  error code.
...
1. **G1 fix — the one substantive residual: not re-tested by a final adversary pass.** D-K's ask-back
   ...
   round probed it. **Recommend:** a fresh adversarial pass — or the build's Playwright verification —
   exercise the **counted + forgets + 0-staff-action** ask-back path specifically.

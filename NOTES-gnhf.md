...
out as each game's `manifest.json`**. The games' loading code is unchanged - they still `fetch('./manifest.json')` and read `folders` / `images` /
...
1. Teach the three games to render `emoji` + `glyphKind` directly, and drop
   ...
   emoji stack).
2. Only then, as its own revertible commit, delete the duplicated trees - checking every key in `provenance.json` first. **Blocked on Stage 3**: see
   below.
...
A generated `<game>/manifest.json` cannot hold anything a technician changes - the next `npm run stimuli:build` overwrites it. Everything AdminTools writes now
...
have left the real admin path writing `_Resources` paths into a generated file - which is exactly the failure Stage 3 exists to close, still fully live.
...
manifest is a *projection*, so one upload can move rows no operation named - replacing an emoji placeholder changes that stimulus's URL for all three games,
...
restore is byte-for-byte the manifest that existed before the archive - `generated` included, because the stamp is a content digest.
...
So a rename writes `shared/stimuli/topics.json` - `game -> category -> name` - and `projectManifest` emits it as `topicNames` in each game's manifest. Nothing
...
`newFolder` from the admin page, and using the name as the option `value` - each fails 1-3 tests.
...
Six kitchen items - blender, microwave, napkin, oven, refrigerator, toaster - have no honest glyph in Unicode. They all ship photographs, so no fallback is
...
boot". Two did: `matching` and `market`. The other eight now do as well - `clock`, `emotions`, `ffc`, `intraverbal`, `patterns`, `receptive`, `sequences`
...
The pairing is the opposite of what the folder names suggest (`m`atching`g`? no - `matching/game.js:242` reads `mgSettings`, `market/game.js:218` reads
...
folds the retired `seqSettings` into the `{sets, last, working}` round store - read-then-fold, never drop - which is the pattern Stage 6 extracts for the other
...
shape of "a setting was silently removed" that hard constraint 1 forbids - fails 3 tests and nothing else; without the toggle assertion it fails none,
...
Both games read `currentTokens` as `s.currentTokens ?? state.startingTokens` - a default derived from another field, which the spec supports via a thunk. But
...
`minimal` would pass against an enum narrowed to those two; seeding `light` - reachable only from `#sel-anim-tier` - is what fails. Narrowing the enum to
...
These are the first two adopters that are **not** library-stimulus games - `intraverbal` draws from its own `items.json` and `patterns` from
...
`patterns` clamps `blanksToFill` to `state.patternLength`, not to a constant - the gift box holds ≤9 tiles, so 3 blanks is legal at pattern length 3 and
...
rows gained a `storeKey` so the seeded retired payload is now asserted twice - returned byte-for-byte under the retired key **and** folded into
...
`el.selCategory.value` to a missing option, which the browser resolves to `''` - and `buildDeck()` then filters on `cat !== 'all' && s.cat !== cat`, matches zero
...
That is cosmetic in the nine games that keep their configuration in `state` - the panel blanks, the programme still runs the stored delay. In `think-or-say`
...
patterns, ffc, think-or-say**. `emotions` is not one of them and cannot be - it has no Prompt Delay boolean at all, because its `promptDelay` *is* the
...
"Delayed" to confirm the procedure they are already on, silently loses the 7 - exactly what hard constraint 1 forbids, arriving through a button that looks
like a no-op.

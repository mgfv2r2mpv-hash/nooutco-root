...
// ── ffc's items, joined to the shared library ──────────────────────
 ...
 * `stimuli.json` at run time.
 *
 * That is deliberate, and it is what keeps ffc out of the Worker's business. An
 * AdminTools upload rewrites a stimulus's URL and deletes the file behind the
 * old one; a URL frozen into this document would point at a file that no longer
 * exists until the next rebuild. Resolving by id instead means ffc follows every
 * upload, label and removal with no re-projection to keep in sync.
 *
 * @param {object} index  the library index, for the id check only
 * @param {object} source `shared/stimuli/ffc.json`
 */
export function projectFfcItems(index, source, { stamp }) {
  const known = new Set(index.stimuli.map((entry) => entry.id));
  const items = [];
  const idAliases = {};

  for (const item of source.items) {
    // A word whose id is not in the library would simply vanish from the game,
    // silently and at run time. The build refuses to publish one.
    if (!known.has(item.id)) throw new Error(`ffc: "${item.legacyId}" names ${item.id}, which the library does not have`);
    if (item.legacyId in idAliases) throw new Error(`ffc: "${item.legacyId}" is listed twice`);
    idAliases[item.legacyId] = item.id;
    items.push({
      id: item.id,
      groups: item.groups,
  ...
  return {
    ...
    library: `${SITE_BASE}stimuli.json`,
    promptDefaults: source.promptDefaults,
    vocab: source.vocab,

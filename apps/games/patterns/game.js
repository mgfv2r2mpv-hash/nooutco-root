...
/* ══════════════════════════════════════════════════════════════════
   PATTERN PACK CO.  (skinned Sequences & Patterns)

   Same trial engine as the unskinned Sequences game, with three
   differences:
     • settings are capped (length ≤3, reps ≤2, blanks ≤3) so the gift
       box never holds more than 9 tiles and the tray always looks good;
     • the trial lifecycle calls into window.PPCSkin (skin.js) for the
       conveyor / lid / bow / neon animations;
     • bank taps are gated by state.locked while an animation is playing.
   ══════════════════════════════════════════════════════════════════ */

// Convenience: a no-op-safe handle to the skin layer.
const Skin = {
  playIntro:  (cb)               => (window.PPCSkin ? window.PPCSkin.playIntro(cb)               : (typeof cb === 'function' && cb())),
  onCorrect:  (boxEl, sym, btn)  => (window.PPCSkin && window.PPCSkin.onCorrect(boxEl, sym, btn)),
  onWrong:    (boxEl, btn)       => (window.PPCSkin && window.PPCSkin.onWrong(boxEl, btn)),
  onComplete: (outcome, cb)      => (window.PPCSkin ? window.PPCSkin.onComplete(outcome, cb)     : (typeof cb === 'function' && cb())),
};
...
const state = {
  ...
  // Skin: blocks bank taps while a conveyor / lid animation plays.
  locked:            false,
...
const el = {
  ...
  chkReduceMotion: $('chk-reduce-motion'),
...
function loadSettings() {
  ...
  el.chkReduceMotion.checked    = state.reduceMotion;
...
function bindEvents() {
  ...
  el.inpPatternLen.addEventListener('change', () => {
    state.patternLength = clamp(parseInt(el.inpPatternLen.value, 10) || 2, 2, 3);
    el.inpPatternLen.value = state.patternLength;
  ...
  el.inpReps.addEventListener('change', () => {
    state.shownReps = clamp(parseInt(el.inpReps.value, 10) || 2, 1, 2);
    el.inpReps.value = state.shownReps;
    saveSettings();
  });
  ...
  el.chkReduceMotion.addEventListener('change', () => {
    state.reduceMotion = el.chkReduceMotion.checked;
    document.body.classList.toggle('reduce-motion', state.reduceMotion);
    saveSettings();
  });
...
function beginTrial(keepUnit = false, isRetry = false) {
  ...
  // Roll the box in on the belt and flip the bulk lid open before any
  // bank taps or prompts are allowed.
  state.locked = true;
  Skin.playIntro(() => {
    state.locked = false;
    schedulePrompt(keepUnit, isRetry);
  });
}

// Auto-prompt scheduling, deferred until the bulk lid has opened so the
// highlight isn't wasted behind a closed lid.
function schedulePrompt(keepUnit, isRetry) {
  if (keepUnit && !isRetry) {
...
function onBankPick(sym, btnEl) {
  ...
  if (state.locked) return;          // belt / lid animation in progress
...
function onCorrectPick(sym, btnEl) {
  ...
  if (blankBox) {
    ...
    // Slide the chosen item from the bulk bin into its slot.
    Skin.onCorrect(blankBox, sym, btnEl);
  }
...
const RESULTS_KEY = 'nooutco.results.patterns';

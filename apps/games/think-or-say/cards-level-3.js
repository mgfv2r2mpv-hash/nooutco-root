/* ── Think or Say? - Level 3 "Explain" ─────────────────────────────────
   ...
   THINK, THINK, SAY, SAY in blocks of four, ten blocks and a tail of two
   (THINK, then SAY) - 42 cards, 21 THINK and 21 SAY.
   ----------------------------------------------------------------------- */
(function (global) {
  'use strict';
  ...
  global.ThinkOrSayLevel3 = {
    ...
    cards: [
      ...
      // The third round, L3-33..L3-42, teaches the ORDER of the rules with
      // three conflict TRIPLETS (RESEARCH.md §10.4) and one lone card:
      //   (a) a base card that a lower rule decides;
      //   (b) the same scene with one HIGHER-rule fact added, and the answer
      //       flips - registered as a pair wherever definePairs accepts it;
      //   (c) the (b) scene with a LOWER-rule fact changed, and the answer does
      //       NOT flip, because the higher rule still decides it. (c) cannot be
      //       a formal pair (pairs must flip), so each triplet is named here.
      // The (c) reasons name the rule that decided AND the lower rule that did
      // not matter ("even though ..."). The rationales carry the deictic frame
      // past I-YOU into NOW-THEN on the timing cards and HERE-THERE on the
      // audience cards (Barron et al. 2018; Chastain & Dixon 2024).
      //
      // ── block 9 ──
      // L3-42 holds no triplet: it keeps the round at five and five. A stranger
      // and a whole line listening are both reasons to stay quiet; everyone
      // hearing is the higher one, so it decides. Nobody is in danger, so the
      // override is declared at none: the reason says so.
      { id: 'L3-42', level: 3, cat: 'other', answer: 'think',
        situation: 'You are waiting in line at the grocery store. A grown-up you have never met is talking on the phone so loudly that the whole line can hear every word.',
        utterance: 'You are talking really loudly.', sayVerb: 'say', object: 'these words',
        features: { audience: 'others-hear', relationship: 'stranger', override: 'none' },
        vary: { setting: 'shop', person: 'stranger', topic: 'body', form: 'statement' },
        reason: 'Think it. Nobody is in danger, and the whole line would hear you, so saying it would embarrass them - and they are a grown-up you do not know. If it bothers you, tell your own grown-up quietly.',
        rationales: [
          'Here, in front of everyone in line, you would feel embarrassed if I said it.',
          'I do not know you, so it is not my job to tell you how loud you are.',
          'I can tell my own grown-up quietly, so only they hear me.',
        ] },
      ...
      // held here instead. L3-39 also declares override: none, which its
      ...
      { id: 'L3-39', level: 3, cat: 'other', answer: 'say',
        ...
        rationales: [
          ...
          'You know where the books are, so asking you helps me find them.',
        ] },
      // TRIPLET 2 - safety over a secret, and over a bad moment.
      //   (a) L3-36  the surprise party, Mom asks at bedtime   THINK (privacy)
    ...
    pairs: [
      ...
      // The third round's triplets. Only (a)/(b) can be a pair, and only where
      // the higher rule has a value that leans neither way (override: none).
      { dim: 'override',      a: 'L3-33', b: 'L3-34' },
      { dim: 'override',      a: 'L3-36', b: 'L3-37' },
    ],
  };
